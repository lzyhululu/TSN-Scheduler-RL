...
class ProcessingModel:
    ...
    def __init__(self, env, handle, name, port, sample_buffer_capacity=1000,
        ...
        addr = 'tsn-pipe-' + str(port)  # named pipe
    ...
    def sample_step(self, rewards, block=True):
        ...
        """
        package = NDArrayPackage(rewards)
...
def model_client(addr, sample_buffer_capacity, RLModel, model_args):
    ...
    sample_buffer = EpisodesBuffer(capacity=sample_buffer_capacity)
    ...
    while True:
        ...
        if cmd[0] == 'act':
            ...
            sample_buffer.record_step(ids, obs, acts, rewards)
            conn.send("done")
        elif cmd[0] == 'save':
            savedir = cmd[1]
            n_iter = cmd[2]
