from frame_demo.data_structure_for_frame import *
import json
...
# 输入：topo_set.json
# 输出：包含Link对象的列表
def _init_link_obj_set_for_frame_demo(filename):
    topo_set = json.load(open('../DataSaved/{}/topo_set.json'.format(filename)))
    # print(topo_set)
    link_obj_set = []
...
def _init_stream_obj_set_for_frame_demo(filename, link_obj_set):
    stream_set = json.load(open('../DataSaved/{}/stream_set.json'.format(filename)))
    stream_obj_set = []
    # print(stream_set)
...
def _init_stream_instance_obj_set_for_frame_demo(stream_obj_set, link_obj_set):
    ...
    for stream_obj in stream_obj_set:
        ...
        for link_id in route_set:
            ...
            stream_instance_obj = StreamInstance(stream_id=stream_id,
                                                 link_id=link_id,
                                                 hop_id=hop_id,
                                                 period=period,
...
def init_topo_and_stream_obj_set_for_frame_demo(filename):
    link_obj_set = _init_link_obj_set_for_frame_demo(filename)
    stream_obj_set = _init_stream_obj_set_for_frame_demo(filename, link_obj_set)
...
def _main():
    init_topo_and_stream_obj_set_for_frame_demo('main_env_1')
    return
