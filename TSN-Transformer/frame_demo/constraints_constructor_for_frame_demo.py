...
def construct_constraints_for_frame_demo(link_obj_set,
    ...
    hyper_period = args.global_cycle

    # 1. frame constraint

    for stream_instance_obj_set_per_stream in stream_instance_obj_set:
        unactivate = stream_instance_obj_set_per_stream[0].unactivate
        ...
        for i in range(len(stream_set)):
            for j in range(len(stream_set)):
                if i != j:
                    ...
                    # 是否考虑此条流
                    i_unactivate = stream_instance_obj_set[i_stream_id][i_hop_id].unactivate
                    j_unactivate = stream_instance_obj_set[j_stream_id][j_hop_id].unactivate
                    ...
                    for alpha in range(math.ceil(hyper_period / i_period)):
                        for beta in range(math.ceil(hyper_period / j_period)):
                            ...
                            formula = Or(i_unactivate, j_unactivate, formula)
                            constraint_formula_set.append(formula)

    # 3.flow transmission constraint
    for stream_instance_obj in stream_instance_obj_set:
        unactivate = stream_instance_obj[0].unactivate
...
def _main():
    ...
    # (link_obj_set,
    #  stream_obj_set,
    #  stream_instance_obj_set) = init_topo_and_stream_obj_set_for_frame_demo('main_env_1')
    #
