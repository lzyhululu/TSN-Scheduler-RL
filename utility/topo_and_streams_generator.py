...
# 输出：拓扑图 DiGraph
def _generate_topo_graph(sw_num,
                         sw_links,
                         es_num_per_sw_set,
                         show_topo_graph):
    # 网络拓扑
    ...
    for sw_link in sw_links:
        G.add_edge(sw_link[0], sw_link[1], link_id=link_id)
        link_id += 1
        G.add_edge(sw_link[1], sw_link[0], link_id=link_id)
        link_id += 1
...
def _generate_topology(sw_num,
    ...
    topo_set = []
    G = _generate_topo_graph(sw_num, sw_links, es_num_per_sw_set, show_topo_graph)
...
# 生成拓扑和流量，并写入文本文件
# 输入：拓扑需求和流量需求
def construct_topo_and_streams(filename,
                               generator,
    ...
    # 第一步：生成拓扑
    G, topo_set = _generate_topology(generator.node_num,
                                     generator.node_links,
                                     es_num_per_sw_set,
                                     speed_set,
                                     st_queues_set,
                                     **kwargs
    ...
    # 第二步：将拓扑信息输出到json文件
    generator.topo_set = topo_set
    generator.write_topo_to_file(filename)
    ...
    stream_set = _generate_random_streams(G, stream_num, size_set,
                                          ...
                                          jitter_requirement_set)
    # 第四步：将流量信息输出到文本文件
    generator.stream_set = stream_set
    generator.write_stream_to_file(filename)
...
def main():
    ...
    # data_gene.write_to_file(filename=args.data_path)
    # data_gene.read_from_file(filename='Vehicle_NetWork')

    construct_topo_and_streams(filename='Vehicle_NetWork',
                               generator=data_gene,
                               es_num_per_sw_set=[2],
                               speed_set=[1000],
                               st_queues_set=[2],
